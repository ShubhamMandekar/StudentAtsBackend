...
//	}
	...
	@Override
	public String incrementstudentattendance(Long id) {

        Optional<User> upi=userRepo.findById(id);
		int inc=upi.get().getAttended_lectures();
		 inc=inc+1;
		userRepo.incattendance(id,inc);

		return "succesfully incremented";

	}

	@Override
	public String decrementStudentattendance(Long id) {

		Optional<User> upi=userRepo.findById(id);
		int dec=upi.get().getAttended_lectures();
		dec=dec-1;
		userRepo.decattendance(id,dec);

		return "succesfully decremented";
	}
